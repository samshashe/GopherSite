...
func TestNewUserNoEmail(t *testing.T) {
	_, err := NewUser("user", "", "password")
	if err != errNoEmail {
		t.Error("Expected err to be errNoEmail")
	}
}

func TestNewUserShortPassword(t *testing.T) {
	_, err := NewUser("user", "user@example.com", "aaa")
	if err != errPasswordTooShort {
		t.Error("Expected err to be errPasswordTooShort")
	}
}
